...
export function AgentsPage() {
  ...
  const [selected, setSelected] = useState<string | null>(fieldAgents[0]?.code ?? null);
  ...
  const selectedAgent = agents.find((agent) => agent.code === selected) ?? agents[0];
      ...
      <div className="grid grid-cols-1 gap-5 2xl:grid-cols-[minmax(0,1fr)_340px]">
        <div className="grid grid-cols-1 gap-4 xl:grid-cols-2 2xl:grid-cols-3">
          {visibleAgents.map((agent) => (
            <article key={agent.code} className={classNames("rounded-[14px] border bg-white p-5 shadow-[0_1px_2px_rgba(7,24,63,0.04)] transition", selected === agent.code ? "border-[#1454c8] ring-4 ring-[#1454c8]/10" : "border-[#dfe7f2] hover:border-[#c8d5e8]")}>
              ...
              <div className="mt-5 flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <button onClick={() => setSelected(agent.code)} type="button" title="Focus agent" className="grid h-9 w-9 place-items-center rounded-xl bg-[#edf4ff] text-[#1158d4]">
        ...
        <Panel className="h-fit">
          <PanelHeader title="Focused Agent" subtitle="Live workload and current assignment" />
          {selectedAgent ? (
            <div className="p-5">
              <div className="flex items-center gap-3">
                <div className="grid h-14 w-14 place-items-center rounded-xl bg-[#1454c8] text-lg font-bold text-white">{selectedAgent.initials}</div>
                <div>
                  <h3 className="font-bold text-[#07183f]">{selectedAgent.name}</h3>
                  <p className="text-sm font-medium text-[#62728b]">{selectedAgent.branch}</p>
                </div>
              </div>
